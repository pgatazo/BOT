...
if authentication_status:
    ...
    def sugestao_formacao(eventos):
        # Exemplo simples: deteta trocas de avançado por médio/defesa para sugerir mudança de sistema
        if not eventos:
            return ""
        ultimo = eventos[-1]
    ...
    def interpretar_tatica(eventos, live_base, resultado_actual):
        # Comentário tático adaptativo (melhorado)
        comentario = ""
        ...
        # Acrescenta sugestão automática:
        comentario += "\n" + sugestao_formacao(eventos)
        return "🤖 Treinador ChatGPT: " + comentario

    # ======= Cálculo de xG Live (podes adaptar) =======
    ...
    with tab2:
        ...
        resultado_actual = 0  # Ajusta se quiseres!
        ...
        if st.button("Exportar para Excel (Live)"):
            ...
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                ...
                df_eventos.to_excel(writer, sheet_name='Eventos')
            st.download_button(
